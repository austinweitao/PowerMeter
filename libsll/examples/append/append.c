    ...
    uci_foreach_element(&pkg->sections, e)
    {
		...
		if(!strcmp("attr",s->type))
		{
        	if (NULL != (value = uci_lookup_option_string(ctx, s, "modbus_id")))
        	{
            		...
            		printf("this attr belongs to meter: modbus_id %d.\n",modbus_id);
					
					for(lp = head;lp;lp = lp->next){
						meter = (Meter *)lp->data;
						if(meter->modbus_id == modbus_id)  //got the right meter
							break;
					}
			}
...
bool load_meter_config()
{
	...
	if(!strcmp("meter",s->type)) //this section is a meter
	{
        	...
        	{
            		addr_commodity = strdup(value); 
            		printf(" commodity is %s.\n",addr->commodity);
        	}
    		addr->attribute = (Meter_Attribute *) malloc(addr->num_attribure * sizeof(Meter_Attribute));
