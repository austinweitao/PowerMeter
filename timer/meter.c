...
#define REMOTE_URL      "ftp://austin:weitao@192.168.148.137:990/Meter-Datas/"  UPLOAD_FILE_AS
...
static void freeData(void **data)
{
    ...
    zlog_debug(zc,"in free data function.\n");
    ...
    if (*addr)
    {
		zlog_debug(zc,"Freeing Meter Attributes First.\n");
    	int i;
		for(i = 0; i< (*addr)->num_attribute; i++){
			if( (*addr)->attribute)
			{
				zlog_debug(zc,"Freeing value_type and value unit.\n");
				...
				zlog_debug(zc,"Freeing attribute itself.\n");
				(void)free((Meter_Attribute *)(*addr)->attribute);
			}
		}
		zlog_debug(zc,"Freeing Meter.\n");
...
void meter_init(void)
{
    ...
    zlog_debug(zc,
"=========================================================================\n");
    zlog_debug(zc," Allocate a new Meter\n");
    zlog_debug(zc,
"=========================================================================\n");
    ...
    addr->modbus_id = 2;
	char path[64];
	sprintf(path,"/tmp/sbs%d_last_sample",addr->modbus_id);
	addr->last_sample_path = strdup(path);
	if(addr->last_sample_path == NULL)
		zlog_debug(zc,"failed to allocate meter commodity.\n");
    ...
    {
		zlog_debug(zc," Attribute:malloc failed\n");
    	destroyNodes(&head,freeData);
		exit(-1);
    }
    ...
    if (tmp->value_type == NULL || tmp->value_unit == NULL) 
    {
		...
		zlog_debug(zc,
"=========================================================================\n");
    	zlog_debug(zc," meter attributes are as follow\n");
    	zlog_debug(zc,
"=========================================================================\n");
    ...
    zlog_debug(zc,
"=========================================================================\n");
    zlog_debug(zc," Testing Append a node at the beginning of a list\n");
    zlog_debug(zc,
"=========================================================================\n");
...
void interval_init(Interval *interval)
{
	...
	zlog_debug(zc,"about to initialize sample interval and upload interval\n");
	interval->sample_interval = get_sample_interval();
	interval->upload_interval = get_upload_interval();

	zlog_debug(zc,"sample interval:%d\n",interval->sample_interval);
	zlog_debug(zc,"upload interval:%d\n\n",interval->upload_interval);
...
static size_t read_callback(void *ptr, size_t size, size_t nmemb, void *stream)
{
  ...
  zlog_debug(zc, "*** We read %" CURL_FORMAT_CURL_OFF_T
          " bytes from file\n", nread);
  return retcode;
}
...
static void upload_file(char *file_to_upload, char *rename_to)
{
	...
	if(stat(file_to_upload, &file_info)) {
		zlog_debug(zc,"Couldnt open '%s': %s\n", LOCAL_FILE, strerror(errno));
		exit -1;
	}
	fsize = (curl_off_t)file_info.st_size;
				  
	
	zlog_debug(zc,"Local file size: %" CURL_FORMAT_CURL_OFF_T " bytes.\n", fsize);
	...
	if(curl) {
		...
		/* Check for errors */
		if(res != CURLE_OK)
			zlog_debug(zc, "curl_easy_perform() failed: %s\n",
...
*/
    ...
    zlog_debug(zc,
"=========================================================================\n");
    zlog_debug(zc," Sampling timer expired\n");
    zlog_debug(zc,"UTC: %2d:%02d:%02d\n", info->tm_hour, info->tm_min,info->tm_sec);
    zlog_debug(zc,"LOCAL: %2d:%02d:%02d\n", info_local->tm_hour, info_local->tm_min,info_local->tm_sec);
    zlog_debug(zc,
"=========================================================================\n");
...
//*****************************sampling data**********************//
    ...
    for (l=head; l; l=l->next)
    {
		...
		if(counter == 1){
			...
			zlog_debug(zc,"the file path is %s.\n",meter->file_path);
			zlog_debug(zc,"the tmp file path is %s.\n",meter->file_tmp_path);
			zlog_debug(zc,"the file path is %s.\n",meter->file_path_xml);
			zlog_debug(zc,"the tmp file path is %s.\n",meter->file_tmp_path_xml);
			zlog_debug(zc,"the file path is %s.\n",meter->file_path_csv);
			zlog_debug(zc,"the tmp file path is %s.\n",meter->file_tmp_path_csv);
		}
		zlog_debug(zc,"opening file %s.\n",meter->file_tmp_path);
		zlog_debug(zc,"opening file %s.\n",meter->file_tmp_path_xml);
		zlog_debug(zc,"opening file %s.\n",meter->file_tmp_path_csv);

		meter->last_sample = fopen(meter->last_sample_path,"w");			
		if(meter->last_sample == NULL)
			zlog_debug(zc,"fopen failed:\n");
...
//**********************************************************************************************//
		...
		*/
    	...
    	if (ctx == NULL) {
       	 	zlog_debug(zc, "Unable to allocate libmodbus context\n");
        	pthread_exit(NULL);
    	}
    	...
    	if (modbus_connect(ctx) == -1) {
        	zlog_debug(zc, "Connection failed: %s\n", modbus_strerror(errno));
        	modbus_free(ctx);
			pthread_exit(NULL);
    	}
		...
		zlog_debug(zc,"num_attribute is %d.\n",meter->num_attribute);
		for(i = 0; i < meter->num_attribute && attribute; i++,attribute++){
    			/* Single register */
				zlog_debug(zc,"reading register.\n");
    			rc = modbus_read_registers(ctx, attribute->addr,
                               attribute->reg_num, tab_rp_registers);

    		if (rc == attribute->reg_num) {
				//update the last sample first
				fprintf(meter->last_sample,"%f ",modbus_get_float_cdab(tab_rp_registers));
				...
				fflush(meter->file);
        			...
        			zlog_debug(zc,"FAILED (nb points %d)\n", rc);

		}
		fprintf(meter->file,"\n");
		fprintf(meter->last_sample,"\n");

		if(counter == get_upload_interval() / get_sample_interval())
			fprintf(meter->file_xml,"  </action>\n</XML>\n");
		zlog_debug(zc,"closing file %s.\n",meter->file_tmp_path);
		zlog_debug(zc,"closing file %s.\n",meter->file_tmp_path_xml);
		zlog_debug(zc,"closing file %s.\n",meter->file_tmp_path_csv);
		zlog_debug(zc,"closing file %s.\n",meter->last_sample_path);
		fclose(meter->file);
		fclose(meter->file_xml);
		fclose(meter->file_csv);
		fclose(meter->last_sample);
...
//******************************end of sampling data******************//
		...
		if(counter ==  get_upload_interval() / get_sample_interval() )
		{
			...
			if( system(system_arguments) != 0) 
				zlog_debug(zc,"system call error.\n"); 

		}
	}
		if(counter ==  get_upload_interval() / get_sample_interval() )
		{
		  	zlog_debug(zc,"######upload time######.\n");
			counter = 0;
    		for (l=head; l; l=l->next)
    		{
				meter = (Meter*) l->data;
...
close:
    	...
    	zlog_debug(zc,"closing the modbus\n");
...
void timer_thread_upload(union sigval v)
{
    	time_t rawtime;
    	struct tm *info;
		zlog_debug(zc,"######upload timer expired######.\n");
    	time(&rawtime);
    	/* Get GMT time */
    	info = gmtime(&rawtime );
    	zlog_debug(zc,"UTC: %2d:%02d:%02d\n", info->tm_hour, info->tm_min,info->tm_sec);
...
int main()
{
	...
	char interval_string[16];
	second_trans(get_sample_interval(),interval_string);
    zlog_debug(zc,"sample interval string is %s\n",interval_string);
	...
	if (timer_create(CLOCKID, &evp_sample, &timerid_sample) == -1) {
		zlog_debug(zc,"fail to sample timer_create\n");
		exit(-1);
	}
	...
	info = gmtime(&rawtime );
	info_local = localtime(&rawtime);
	zlog_debug(zc,"Current world clock:\n");
	zlog_debug(zc,"UTC: %4d%02d%02d%02d%02d%02d\n\n", (info->tm_year + 1900),(info->tm_mon + 1),info->tm_mday,info->tm_hour, info->tm_min,info->tm_sec);
	zlog_debug(zc,"Current local clock:\n");
	...
	if (timer_settime(timerid_sample, 0, &it_sample, NULL) == -1) {
		zlog_debug(zc,"fail to sample timer_settime\n");
		exit(-1);
	}
#if 0
